...
async function removeZego(parsedReactNativeConfig, extraModules) {
  await addForceUnlinkForNativePackage(
...
async function main() {
  ...
  try {
    ...
    if (apptileConfig.feature_flags?.ENABLE_LIVELY) {
      // TO DO: ADD MICROPHONE AND CAMERA ACCESS PERMISSIONS
      console.log('Not doing anything for zego');
    } else {
      console.log('Removing zego');
      await removeZego(parsedReactNativeConfig, extraModules);
    }
