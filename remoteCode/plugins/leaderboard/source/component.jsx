...
export function ReactComponent({ model }) {
  ...
  const fetchLeaderboard = async () => {
    try {
      ...
      console.log('[LEADERBOARD] URL:', `${SUPABASE_URL}/rest/v1/users?select=*&order=total_points.desc,total_potholes.desc`);

      const response = await fetch(
        `${SUPABASE_URL}/rest/v1/users?select=*&order=total_points.desc,total_potholes.desc`,
        {
          headers: {
