...
17. [Integration: Loop Subscriptions](#integration-loop-subscriptions)
...
- `subscriptionId`, `orderId`, etc. — pass via navigation params
...
navigation.navigate('TargetScreenName', { subscriptionId: '123' });
...
```javascript
async function callApi(baseUrl, apiKey, token, id, payload) {
  console.log('[Loop] callApi', id, JSON.stringify(payload));
  ...
  if (!res.ok) {
    ...
    console.log('[Loop] callApi FAILED', res.status, errBody);
  ...
  const json = await res.json();
  console.log('[Loop] callApi OK', JSON.stringify(json).substring(0, 500));
  return json;
}
```
...
const performAction = useCallback(async (actionFn, successMsg) => {
  ...
  try {
    ...
    await loadSubscription(loopToken); // refresh after mutation
  } catch (e) {
...
// Every action that modifies subscription state MUST use this
<TouchableOpacity onPress={() => confirmAndExecute('Skip?', 'Skip the next order?', handleSkip)}>
```
...
- ❌ Putting `subscriptionId` in WidgetEditors
...
Always fetch valid options (frequencies, plans, intervals) from the API. Never hardcode what could vary per merchant.
...
Always use `Alert.alert` before: skip, pause, resume, cancel, place order, quantity changes.
...
renderSection('Delivery Frequency', content, 'frequency', (
...
// In section render:
editMode === 'frequency' ? (
  <FrequencyEditForm onSave={handleSave} onCancel={() => setEditMode(null)} />
) : (
  <Text>{currentFrequency}</Text>
)
```
...
## Integration: Loop Subscriptions

### Plugins

| Plugin | Screen Name | Purpose |
|--------|-------------|---------|
| `loopactivesubscriptions` | `LoopActiveSubscriptions` | Lists active & past subscriptions (tabbed). Tap card → Manage. |
| `loopmanagesubscription` | `LoopManageSubscription` | Manage a single subscription (pause, resume, skip, edit frequency/address/notes/discounts, line items). |
| `loopcancelsubscription` | `LoopCancelSubscription` | 3-step cancellation flow (Detail → Confirm → Success). |

### Navigation Flow
```
Active Subscriptions → [tap card] → Manage Subscription → [cancel btn] → Cancel Subscription
         ↓ passes subscriptionId            ↓ passes subscriptionId
```

### Authentication (2-Step: Admin API → Storefront JWT)
```
1. POST /admin/2023-10/customer/{customerShopifyId}/sessionToken
   Header: X-Loop-Token: <adminApiKey>
   → returns sessionToken

2. POST /storefront/2023-10/auth/refreshToken
   Body: { sessionToken }
...
**Why not Shopify App Proxy?** `POST {storeDomain}/a/loop_subscriptions/...` requires native Shopify session cookies, which are unavailable in Apptile.

### Merchant-Required Config
| Property | Editor Type | Used By |
|----------|-------------|---------|
| `apiKey` | `codeInput` | All 3 plugins |
| `baseUrl` | `codeInput` | All 3 plugins |

### NOT Exposed (Auto-Managed)
| Property | Source |
|----------|--------|
| `customerShopifyId` | Redux: `shopify.loggedInUser.id.split('/').pop()` |
| `subscriptionId` | React Navigation params |

### API Base URL
`https://api.loopsubscriptions.com` — endpoints prefixed `/storefront/2023-10/` or `/admin/2023-10/`

### Loop Storefront API Endpoints

| Endpoint | Method | Purpose |
|----------|--------|---------|
| `/storefront/2023-10/subscription` | GET | List all customer subscriptions |
| `/storefront/2023-10/subscription/{id}` | GET | Get subscription detail |
| `/storefront/2023-10/subscription/{id}/pause` | POST | Pause subscription |
| `/storefront/2023-10/subscription/{id}/resume` | POST | Resume paused subscription |
| `/storefront/2023-10/subscription/{id}/skipNext` | POST | Skip next order |
| `/storefront/2023-10/subscription/{id}/placeOrder` | POST | Place order immediately |
| `/storefront/2023-10/subscription/{id}/reschedule` | POST | Body: `{ newBillingDateEpoch, timezone? }` |
| `/storefront/2023-10/subscription/{id}/frequency` | GET | List available frequencies |
| `/storefront/2023-10/subscription/{id}/frequency` | PUT | Body: `{ billingPolicy: { intervalCount, interval }, nextBillingDateEpoch }` |
| `/storefront/2023-10/subscription/{id}/address` | PUT | Update shipping address |
| `https://api.loopwork.co/api/customer/v2/edit_order_note` | POST | Body: `{ subscriptionContract_shopify_id, note }`. Bearer auth only, no x-api-key. |
| `/storefront/2023-10/subscription/{id}/discount` | POST | Body: `{ code }` |
| `/storefront/2023-10/subscription/{id}/discount/{discountShopifyId}` | DELETE | Remove discount code |
| `/storefront/2023-10/subscription/{id}/line/{lineId}/quantity` | PUT | Body: `{ quantity }` |
| `/storefront/2023-10/subscription/{id}/line/{lineId}` | DELETE | Remove line item |
| `/storefront/2023-10/subscription/{id}/line/{lineId}/swap` | PUT | Body: `{ variantShopifyId, quantity, sellingPlanGroupId? }` |
| `/storefront/2023-10/subscription/{id}/line/once` | POST | Add one-time product. Body: `{ variantShopifyId, quantity, ... }` |
| `/storefront/2023-10/subscription/{id}/line` | POST | Add recurring line item |
| `https://api.loopwork.co/api/customer/v2/read_upsell_products` | GET | Upsell products. Bearer only. |
| `https://api.loopwork.co/api/customer/v2/read_swap_products?lineItem_id={lineId}...` | GET | Swap products. Bearer only. |
| `/storefront/2023-10/subscription/{id}/orders?type=scheduled` | GET | List scheduled orders |
| `/storefront/2023-10/subscription/{id}/orders?type=past` | GET | List past orders |
| `/storefront/2023-10/subscription/{id}/cancel` | POST | Body: `{ cancellationFlowId, comment? }` |
| `/storefront/2023-10/subscription/{id}/cancellationFlow` | GET | Get cancellation flow/reasons |
| `/storefront/2023-10/paymentMethod` | GET | List payment methods |
| `/storefront/2023-10/auth/refreshToken` | POST | Exchange session token for access token |
| `/admin/2023-10/customer/{shopifyId}/sessionToken` | POST | Get session token (header: `X-Loop-Token`) |

### Loop Subscription Object — Key Fields
```javascript
{
  id, status, // 'active' | 'paused' | 'cancelled' | 'expired' | 'failed' | 'scheduled'
  price, totalPrice, currencyCode,
  shippingPrice, deliveryPrice,
  billingPolicy: { intervalCount, interval },  // e.g. { intervalCount: 3, interval: 'MONTH' }
  nextBillingDate, nextBillingDateEpoch,
  lastBillingDate, lastOrderDate,
  lastPaymentStatus,                           // 'SUCCESS' | 'FAILED' | etc.
  customerPaymentMethodId,
  shippingAddress: { firstName, lastName, address1, address2, city, province, provinceCode, zip, country, countryCode },
  orderNote,
  discountCodes: [{ code, value }],
  lines: [{
    id, productTitle, variantTitle, quantity,
    price, compareAtPrice,
    productImage, variantImage,
    inventoryQuantity,          // <= 0 means out of stock
    discountPercentage, subscriptionDiscount,
  }],
}
```

### Loop-Specific Issues & Solutions
| Issue | Solution |
|-------|----------|
| Loop App Proxy returns 401 | Use Admin API 2-step auth |
| API response structure varies | Fallback: `data.subscription \|\| data.data \|\| data` |
| Need active vs cancelled tabs | Tabbed interface with status filtering |
| Missing payment method details | `Promise.allSettled` to fetch `/paymentMethod` in parallel |
| Line item quantity needs confirmation | `Alert.alert` before PUT |
| Discount code error messages | Parse response body for user-facing message |
...
- [ ] Dynamic IDs (customerShopifyId, subscriptionId) NOT in WidgetEditors
