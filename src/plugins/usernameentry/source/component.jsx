...
export function ReactComponent({ model }) {
  ...
  const checkExistingUser = async () => {
    try {
      ...
      console.log('[USERNAME_ENTRY] Checking for existing user...');

      const deviceId = await DeviceInfo.getUniqueId();
      console.log('[USERNAME_ENTRY] Device ID:', deviceId);

      const cachedUser = await AsyncStorage.getItem('pothole_user');

      if (cachedUser) {
        const userData = JSON.parse(cachedUser);
        console.log('[USERNAME_ENTRY] Found cached user:', userData);

        // Fetch latest data from Supabase to get updated points
        try {
          const refreshResponse = await fetch(
            `${SUPABASE_URL}/rest/v1/users?device_id=eq.${deviceId}&select=*`,
            {
              headers: {
          ...
          if (refreshResponse.ok) {
            ...
            if (refreshedUsers.length > 0) {
              const refreshedUserData = refreshedUsers[0];
              console.log('[USERNAME_ENTRY] Refreshed user data from DB:', refreshedUserData);

              // Update AsyncStorage with latest data
              await AsyncStorage.setItem('pothole_user', JSON.stringify(refreshedUserData));
              ...
              // User not found in DB, clear cache
              console.log('[USERNAME_ENTRY] ⚠️ Cached user not found in DB, clearing cache');
              await AsyncStorage.removeItem('pothole_user');
              setCheckingUser(false);
              return;
            }
          }
        } catch (refreshError) {
          console.warn('[USERNAME_ENTRY] Failed to refresh user data:', refreshError);
          // If network error, clear cache to be safe
          await AsyncStorage.removeItem('pothole_user');
          setCheckingUser(false);
          return;
        }
      ...
      if (response.ok) {
        ...
        if (users.length > 0) {
          ...
          console.log('[USERNAME_ENTRY] Found existing user in DB:', userData);
          
          await AsyncStorage.setItem('pothole_user', JSON.stringify(userData));
      ...
      console.log('[USERNAME_ENTRY] No existing user found, showing username entry');
      setCheckingUser(false);

    } catch (error) {
      console.error('[USERNAME_ENTRY] Error checking user:', error);
      setCheckingUser(false);
    }
  };
  ...
  const handleLetsGo = async () => {
    ...
    try {
      setLoading(true);
      console.log('[USERNAME_ENTRY] Creating user with username:', username);
      ...
      if (!response.ok) {
        ...
        console.error('[USERNAME_ENTRY] Error creating user:', errorText);
        throw new Error('Failed to create user');
      }

      const users = await response.json();
      const userData = users[0];
      console.log('[USERNAME_ENTRY] ✅ User created:', userData);
      ...
      console.error('[USERNAME_ENTRY] Error:', error);
  ...
  if (checkingUser) {
    return (
      <View
        ...
        style={[styles.loadingContainer, { width: windowWidth, height: windowHeight }]}
      >
  ...
  return (
    <View
      nativeID={'rootElement-' + id}
      style={{ width: windowWidth, height: windowHeight, backgroundColor: '#f5f5f5' }}
    >
      ...
      >
        <ScrollView
          ...
          contentContainerStyle={{ flexGrow: 1, alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 40 }}
          keyboardShouldPersistTaps="handled"
        >
          {/* PotFix Logo and Name - At the top */}
          <View style={styles.logoContainer}>
            <Image
              ...
              resizeMode="contain"
            />
            <Text style={styles.title}>PotFix</Text>
            <Text style={styles.subtitle}>The app for road warriors.</Text>
          </View>

          {/* Input Section - Centered in middle */}
        ...
        <View style={styles.inputContainer}>
          <TextInput
            ...
            autoCorrect={false}
            maxLength={30}
            nativeID="usernameentry-TextInput-Username"
          />
        </View>
        ...
        <TouchableOpacity
          ...
          disabled={loading}
          nativeID="usernameentry-TouchableOpacity-LetsGo"
        >
          {loading ? (
          ...
          {/* Powered by tile - At the bottom */}
          <View style={styles.poweredByContainer}>
            <Text style={styles.poweredByText}>Powered by</Text>
            <SvgXml xml={tileSvg} width={68} height={26} />
          </View>
        </ScrollView>
      </KeyboardAvoidingView>
    </View>
