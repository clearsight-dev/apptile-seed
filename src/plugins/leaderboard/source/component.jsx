...
import { View, Text, ScrollView, ActivityIndicator, RefreshControl, TouchableOpacity, StatusBar, Linking, Pressable } from 'react-native';
...
// X (Twitter) Icon SVG
const xIconSvg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640" fill="#501F16">
  <path d="M453.2 112L523.8 112L369.6 288.2L551 528L409 528L297.7 382.6L170.5 528L99.8 528L264.7 339.5L90.8 112L236.4 112L336.9 244.9L453.2 112zM428.4 485.8L467.5 485.8L215.1 152L173.1 152L428.4 485.8z"/>
</svg>`;

// DiceBear avatar configuration
const AVATAR_STYLE = 'fun-emoji'; // Options: avataaars, lorelei, bottts, personas, fun-emoji

const getAvatarUrl = (username) => {
  if (!username) return null;
  return `https://api.dicebear.com/9.x/${AVATAR_STYLE}/svg?seed=${encodeURIComponent(username)}`;
};
...
export function ReactComponent({ model }) {
  ...
  const [shareBannerData, setShareBannerData] = useState(null);

  const currentUser = appState?.currentUser;

  useEffect(() => {
    fetchLeaderboard();
    checkForUnsharedPothole();
  }, []);

  const checkForUnsharedPothole = async () => {
    try {
      const hasShared = await AsyncStorage.getItem('hasSharedLastPothole');
      const address = await AsyncStorage.getItem('pendingShareAddress');
      const severity = await AsyncStorage.getItem('pendingShareSeverity');

      if (hasShared !== 'true' && address && severity) {
        setShareBannerData({ address, severity });
        setShowShareBanner(true);
      }
    } catch (error) {
      console.error('[LEADERBOARD] Error checking share status:', error);
    }
  };

  const handleShareOnX = async () => {
    if (!shareBannerData) return;
...
#RoadSafety #PotholePatrol`;
      ...
      console.error('[LEADERBOARD] Share error:', error);
    }
  };
    ...
    >
      ...
      <View style={styles.header}>
        <TouchableOpacity
          ...
          nativeID="leaderboard-TouchableOpacity-Back"
        >
          <Icon
      ...
      {showShareBanner && shareBannerData && (
        <View
          style={{
        ...
        >
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
            ...
            <Pressable onPress={closeBanner} style={{ padding: 4 }}>
              <Icon iconType="MaterialIcons" name="close" size={20} color="#92400E" />
            </Pressable>
          </View>
        </View>
      )}
        ...
        <View style={{ flex: 1 }}>
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptyText}>
              No users yet. Be the first to report a pothole!
            </Text>
          </View>

          {/* Powered by Tile - Empty State */}
          <View style={[styles.poweredByContainer, { position: 'absolute', bottom: 40, left: 0, right: 0 }]}>
                  ...
                  return (
                    <View
                      ...
                      style={[
                        ...
                        isThird && styles.podiumItemThird,
                      ]}
                      nativeID={`leaderboard-View-Podium-${actualRank}`}
                    >
                      <View style={styles.avatarContainer}>
                        <View
                          style={[
                            ...
                            isSecond && styles.avatarRingSecond,
                            isThird && styles.avatarRingThird,
                          ]}
                        >
                          <SvgUri
                            uri={getAvatarUrl(user.username)}
                            width={isFirst ? 88 : 78}
              ...
              {rest.map((user, index) => {
                ...
                return (
                  <View
                    ...
                    nativeID={`leaderboard-View-User-${rank}`}
                  >
                    <Text
                      style={[
                        styles.rankNumber,
                        isCurrentUser && styles.currentUserRankNumber,
                      ]}
                    >
                      {rank}
                    </Text>
                    ...
                    >
                      <SvgUri uri={getAvatarUrl(user.username)} width={40} height={40} />
                    </View>
                    <View style={styles.listUserInfo}>
                      <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                        ...
                        {isCurrentUser && (
                          <Text style={styles.listUserLabel}>You</Text>
                        )}
                      </View>
                    </View>
                    <Text
