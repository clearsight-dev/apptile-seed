...
// X (Twitter) Icon SVG
const xIconSvg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640" fill="white">
...
export function ReactComponent({ model }) {
  ...
  const [lastReportedAddress, setLastReportedAddress] = useState('');
  const [lastReportedSeverity, setLastReportedSeverity] = useState('');
  ...
  useEffect(() => {
    const subscription = AppState.addEventListener('change', async (nextAppState) => {
      if (nextAppState === 'active') {
        ...
        const storedSeverity = await AsyncStorage.getItem('lastReportedSeverity');
        const storedAddress = await AsyncStorage.getItem('lastReportedAddress');

        await fetchPotholes();

        // Show points popup if there's a new pothole
        if (lastReportedId && storedSeverity) {
          const points = storedSeverity === 'Insurance Claimer' ? 10 :
                        storedSeverity === 'The Spine Rattler' ? 20 : 30;
          setEarnedPoints(points);
          setLastReportedSeverity(storedSeverity);
          setLastReportedAddress(storedAddress || 'Unknown location');
          setShowPointsPopup(true);

          // Set pending share data for leaderboard banner
          await AsyncStorage.setItem('hasSharedLastPothole', 'false');
          await AsyncStorage.setItem('pendingShareAddress', storedAddress || 'Unknown location');
          await AsyncStorage.setItem('pendingShareSeverity', storedSeverity);

          // Clear the flags
          await AsyncStorage.removeItem('lastReportedPotholeId');
          await AsyncStorage.removeItem('lastReportedSeverity');
          await AsyncStorage.removeItem('lastReportedAddress');
        }
      }
    });
  ...
  useEffect(() => {
    ...
    if (prevCapturedPhotoRef.current && !capturedPhoto) {
      ...
      const checkForPopup = async () => {
        ...
        // Show points popup if there's a new pothole
        if (lastReportedId && storedSeverity) {
          console.log('[MAPVIEW] 🎉 Showing points popup for:', storedSeverity);
          const points = storedSeverity === 'Insurance Claimer' ? 10 :
                        storedSeverity === 'The Spine Rattler' ? 20 : 30;
          setEarnedPoints(points);
          setLastReportedSeverity(storedSeverity);
  ...
  const handleShareOnX = async () => {
    try {
      console.log('[MAPVIEW] Sharing on X...');
...
#RoadSafety #PotholePatrol`;
      ...
      if (canOpen) {
        ...
        // Mark as shared
        await AsyncStorage.setItem('hasSharedLastPothole', 'true');

        // Award 10 points for sharing
        if (currentUser) {
          try {
            const response = await fetch(
              `${SUPABASE_URL}/rest/v1/users?device_id=eq.${currentUser.device_id}`,
              {
                method: 'PATCH',
                headers: {
                  'apikey': SUPABASE_ANON_KEY,
                ...
                body: JSON.stringify({
                  total_points: currentUser.total_points + 10,
                }),
              }
            );
            ...
            if (response.ok) {
              ...
              if (users.length > 0) {
                ...
                console.log('[MAPVIEW] ✅ Share points awarded! New total:', updatedUser.total_points);
              }
            }
          } catch (error) {
            console.error('[MAPVIEW] Failed to award share points:', error);
          }
        }
      } else {
        console.error('[MAPVIEW] Cannot open X URL');
      }
    } catch (error) {
      console.error('[MAPVIEW] Share error:', error);
    }
  };
        ...
        {potholes.map((pothole) => {
          ...
          return (
            <PointAnnotation
              ...
              onSelected={() => {
                const now = Date.now();
                const timeSinceDeselect = now - lastDeselectTime.current;
                const isSameMarkerReselecting = lastDeselectedId.current === pothole.id;

                console.log('[APPTILE_AGENT] Marker selected:', pothole.id, 'Time since deselect:', timeSinceDeselect, 'Same marker:', isSameMarkerReselecting);

                // Prevent flicker: ignore reselection of same marker within 200ms
                if (isSameMarkerReselecting && timeSinceDeselect < 200) {
                  console.log('[APPTILE_AGENT] Ignoring flicker reselection');
                  return;
                }
              ...
              onDeselected={() => {
                ...
                lastDeselectTime.current = Date.now();
                lastDeselectedId.current = pothole.id;
                setSelectedPothole(null);
              }}
            >
              <View
        ...
        >
          ...
          >
            ...
            <Pressable
              ...
              nativeID="mapview-Pressable-ShareX"
            >
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                <Text style={{ color: '#ffffff', fontSize: 16, fontWeight: '600' }}>
                  Share on
                </Text>
                <SvgXml xml={xIconSvg} width={16} height={16} />
              </View>
            </Pressable>
