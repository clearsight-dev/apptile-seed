...
async function downloadIconAndSplash(apptileConfig) {
  ...
  // Generate splash config file
  if (splashFileName) {
    await generateSplashConfig(splashFileName);
  }

  return result;
}

async function generateSplashConfig(splashFileName) {
  const fileExtension = path.extname(splashFileName);
  const isGif = fileExtension.toLowerCase() === '.gif';

  const splashConfigContent = `// This file is generated at build time based on the splash asset downloaded
// DO NOT EDIT THIS FILE MANUALLY

export const SPLASH_FILE_NAME: string = '${splashFileName}';
export const IS_SPLASH_GIF: boolean = ${isGif};
`;

  await writeFile(
    path.resolve(__dirname, 'config/splashConfig.ts'),
    splashConfigContent,
  );
  console.log(`Generated splash config with fileName: ${splashFileName}, isGif: ${isGif}`);
}
