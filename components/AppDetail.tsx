...
const AppDetail: React.FC<ScreenProps> = ({route}) => {
  ...
  const [appDraft, setAppDraft] = useState<
    IAppDraftResponse['appDraft'] | null
  >(null);
  ...
  const [liveCommitData, setLiveCommitData] = useState<ICommitResponse | null>(
    null,
  );
  ...
  async function logLiveBranchInfo() {
    try {
      ...
      const propBranch = branchesData.branches.find(
  ...
  async function fetchPushLogs(appId: string) {
    try {
      ...
      console.error(
        'Failed to fetch pushLogs: ' + (err || 'noerror').toString(),
      );
      dispatch({
        type: 'SET_ERROR',
  ...
  async function fetchLastSavedConfig(
    appId: string,
    ...
    try {
      const savedConfigData = await fetchLastSavedConfigApi(
        appId,
        forkId,
        branchName,
      );
  ...
  async function fetchManifest(appId: string) {
    try {
      ...
      for (let i = 0; i < manifest.forks.length; ++i) {
        ...
        const mainBranchLatestSave = await fetchLastSavedConfig(
          appId,
          fork.id,
          branchName,
        );
  ...
  async function downloadForPreviewNonCache(
    ...
    const zipDownloadPath = getCustomPreviewBundleZipPath(appId); // Path for the downloaded zip file
    const unzipToDir = getCustomPreviewBundlePath(appId); // Directory to unzip the bundle into

    try {
      // Ensure the target directory for unzipping exists
      await RNFetchBlob.fs.mkdir(unzipToDir).catch(err => {
        if (
          err &&
          err.message &&
          ...
          console.error('Failed to create unzip directory:', unzipToDir, err);
      ...
      if (bundleUrl) {
        bundleDownload = RNFetchBlob.config({
          fileCache: true,
          path: zipDownloadPath, // Use the determined zip download path
        }).fetch('GET', bundleUrl);
      }
      ...
      try {
        console.log(`Unzipping from ${zipDownloadPath} to ${unzipToDir}`);
        await unzip(zipDownloadPath, unzipToDir, 'UTF-8');
        ...
        const bundleMainFile = Platform.select({
          ios: 'main.jsbundle',
          android: 'index.android.bundle',
        });
        const actualPreviewBundlePath = `${unzipToDir}/${bundleMainFile}`;
        const appConfigPath = `${RNFetchBlob.fs.dirs.DocumentDir}/appConfig.json`;

        const previewTrackerFileContent = JSON.stringify({
          previewBundle: actualPreviewBundlePath,
          previewAppConfig: appConfigPath,
        });

        // Write to a dynamic previewTracker.json in the app's documents directory
        const previewTrackerJsonPath = `${RNFetchBlob.fs.dirs.DocumentDir}/previewTracker.json`;
        ...
        console.log(
          `Successfully updated previewTracker.json at ${previewTrackerJsonPath} to point to: ${actualPreviewBundlePath}`,
        );
        // --- End of update previewTracker.json ---

        RNRestart.Restart();
      } catch (err) {
        setDraftPreviewLoading(false);
        console.error('Failed to unzip files or update preview tracker:', err);
        dispatch({
          type: 'SET_LAUNCH_SEQUENCE',
          payload: [
            {
    ...
    try {
      ...
      if (appId && publishedCommitId) {
        ...
        try {
          ...
          setLivePreviewLoading(false);
          console.error(
            'Failed to unzip files or update preview tracker:',
            err,
          );
          dispatch({
            type: 'SET_LAUNCH_SEQUENCE',
    ...
    <>
      <ScrollView
        ...
        <AppInfo
          ...
          forkName={
            currentFork?.title == 'Main' || currentFork?.title == 'main'
              ? ''
              : currentFork?.title
          }
          branchName={
            branchTitle == 'Main' || branchTitle == 'main' ? '' : branchTitle
          }
              ...
              <>
                ...
                <View style={styles.versionCard}>
                  <View
                    ...
                    {isLatestPreviewActive &&
                      state.launchSequence.some(
                  ...
                  <Text style={styles.versionDate}>
                    {liveCommitData?.createdAt
                      ? getFormattedDate(liveCommitData?.createdAt)
                      : ''}
                  </Text>
                  <StyledButton
            ...
            <View style={styles.versionCard}>
              {appDraft ? (
                <>
                  ...
                  <Text style={styles.versionDate}>
                    {appDraft?.createdAt
                      ? getFormattedDate(appDraft.createdAt)
                      : ''}
                  </Text>
                  <LauncSequenceModal
                    state={state}
